...
bool AP_Arming_Plane::arm(const AP_Arming::Method method, const bool do_arming_checks)
{
    ...
    // when arming by switch while rudder is full right -> arm without further checks
    if((AP_Arming::Method::AUXSWITCH == method) && (plane.channel_rudder != nullptr)) {
        if (plane.channel_rudder->get_control_in() > 4000){
            do_checks = false;    
        }
    }
