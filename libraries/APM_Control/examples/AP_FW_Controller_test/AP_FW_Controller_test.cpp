...
void loop(void)
{
    ...
    switch (test_axis) {
        case Axis::Roll:
            ...
            output = roll_control.run_angle_control(nav_angle_cd, speed_scaler, disable_integrator, freeze_integrator, ground_mode);
            info = &roll_control.get_pid_info();
            break;

        case Axis::Pitch:
            angle_error_cd = nav_angle_cd - ahrs.pitch_sensor;
            output = pitch_control.run_angle_control(nav_angle_cd, speed_scaler, disable_integrator, freeze_integrator, ground_mode);
